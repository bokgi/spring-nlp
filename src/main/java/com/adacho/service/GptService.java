...
public class GptService {
	...
	public GptResponseDto getGptComment(String userInput, List<Integer> recommandList) {
		...
		String gptInput = "사용자 입력: \"" + userInput + "\"\n" +
                ...
                sb.toString() + "\n\n" + 
                "위 목록에 있는 식당들을 userInput과 연관되어 있는 10개 정도의 식당만 선택하고 설명해주세요.\n" +
                "**중요**: 설명은 반드시 위 목록 순서를 따라야 합니다.\n\n" +
                "**중요**: 결론에는 반드시 '감사합니다.' 만 생성해야 합니다. 다른 글이 생성되면 안 됩니다.\n\n"+
                
                "설명형식은 아래와 같습니다. 반드시 지켜주세요.\n " +
                "- 첫번째 줄에 자연스럽고 친근한 인삿말을 넣습니다.\n " +
                "- 그 다음 줄부터 각 식당의 정보를 설명합니다. 각 설명은 [식당 이름: 식당에 대한 설명 내용] 형태로 시작하며, 전체적으로 **친절하고 자연스러운 대화체 어조**를 사용하되, 각 식당마다 **다양한 표현과 문장 구조**를 사용해야 합니다.\n" +
                "- 설명에 특수문자, 특수효과(기울임,굵게 등) 사용하지 마세요.\n\n" +

        		"위 목록에 있는 식당들을 목록에 나타나는 순서대로 설명해 주세요. 설명은 아래 예시와 같이, **다양하고 흥미로운 어조**로 작성하며 각 식당마다 **다채로운 표현과 문장 구조**를 사용해야 합니다. **식당 설명 사이에 순서를 암시하는 연결 어구(마지막으로, 다음으로, 두 번째 등)는 절대 사용하지 않습니다. \n\n" +
                "--- 설명 형식 예시 시작 ---\n" + 
        		"[자연스럽고 친근한 인삿말]\n" +
                ...
                "식당 이름 3: [간단한 식당 정보 요약 - 예: 조용하고 아늑한 분위기에서 정갈한 한식을 맛볼 수 있는 식당이에요.] [추천 대상 - 예: 부모님을 모시고 가거나 차분하게 식사하고 싶은 분]들께 딱입니다. 평균 평점은 [평점]점이며, 리뷰에서는 [긍정적 피드백 - 예: 정갈한 반찬과 깊은 맛의 국물]에 대한 칭찬이 많았어요. [평점이 낮은 편이라면 이유 포함 - 예: 공간이 넓지 않아 단체 방문은 어려울 수 있습니다.]\n\n" +
